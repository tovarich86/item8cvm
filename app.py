...
def chat_with_data_agent(query: str):
    ...
    # Convertendo os dicionários simples de st.session_state.messages para o formato que GenAI espera.
    # O GenAI espera uma lista de glm.Content objects.
    chat_history_for_gemini = []
    for msg in st.session_state.messages:
        # Cada 'msg' em st.session_state.messages é um dict: {"role": "...", "parts": [{"text": "..."}]}
        # Precisamos converter isso para glm.Content para o histórico do Gemini.
        ...
        if isinstance(msg.get("parts"), list):
            for part_data in msg["parts"]:
                ...
                # Adicione outras chaves (e.g., "function_call", "function_response") se você as armazenar no histórico
                # e se o start_chat precisar delas. Para o momento, focando no texto.
        ...
        try:
            ...
            # Isso captura erros na conversão de uma mensagem específica para glm.Content
            st.error(f"Erro ao converter mensagem para histórico do Gemini: {e}. Mensagem: {msg}")
            # Pode optar por pular esta mensagem ou limpar o histórico.
            # Para depuração, é bom ver a mensagem problemática.
            continue # Pula esta mensagem no histórico, se for inválida


    # Iniciar o chat com o modelo e a instrução do sistema
    # st.write("History sent to start_chat:", chat_history_for_gemini) # Debug
    ...
    for part in final_model_content.parts:
        ...
        # Lidar com function_call/function_response do modelo, se o LLM as gera no meio da conversa
            ...
            simple_parts.append({"text": f"Resposta da ferramenta: {part.function_response.get('text', 'N/A')}"}) # Para exibir ao usuário
...
if user_query:
    ...
    chat_with_data_agent(user_query)import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import io
import base64
